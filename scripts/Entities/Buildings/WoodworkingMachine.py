...
from scripts.Managers.GameAssets import woodworking_machine_recipies
...
TEXTURE = load_texture("assets/entities/woodworking_machine.png", TEXTURE_SIZE)
TEXTURE_OFFSET = TEXTURE_SIZE * Vector(-0.5, -0.8)
TOUCH_HITBOX_SIZE = TEXTURE_SIZE * Vector(1, 0.3)
TIUCH_HITBOX_OFFSET = TEXTURE_SIZE * Vector(0, 0.7)


class WoodworkingMachine(Building, metaclass=EntityRegistry):

    def __init__(self, code, pos, parent):

        super().__init__(code, pos, parent, TEXTURE, 200, TEXTURE_OFFSET, "woodworking_machine", "pickaxe", 8,
                         ui_id="WoodworkingMachineUI", collider_hitbox_size=TOUCH_HITBOX_SIZE, collider_hitbox_offset=TIUCH_HITBOX_OFFSET, forced_updating=True)
    ...
    def update(self, preview_mode: bool = False):
        ...
        if not preview_mode:
            ...
            if self.ing_slot.item_amount > 0 and self.max_progress == 0 and woodworking_machine_recipies.find(self.ing_slot.item_name):

                recipe = woodworking_machine_recipies.find(self.ing_slot.item_name)
