...
folder = "assets/entities/deposits"
#TEXTURES = {file.split('.')[0]: load_texture(os.path.join(folder, file), DEPOSIT_SIZE) for file in os.listdir(folder)}
...
class Deposit(metaclass=EntityRegistry):
    ...
    def __init__(self, code: any, pos: Vector, parent, ore_type: str = "stone"):
        ...
        self.texture = ORE_TEXTURES.get(ore_type, pg.transform.scale(TEXTUTURE_NULL, DEPOSIT_SIZE.as_tuple()))["deposit"]
        self.do_draw = ()
        self.__texture_offset = Vector(-(DEPOSIT_SIZE.x / 2), -(DEPOSIT_SIZE.y / 16 * 15))
        self.tobeddeleted = False
        self.rect = pg.Rect((0, 0, DEPOSIT_SIZE.x, DEPOSIT_SIZE.y))
        self.touching = False
    ...
    def update(self):
        ...
        if any(self.do_draw):
            ...
            try:
                ...
                item_type = "null"
            self.touching = self.rect.collidepoint(get_mouse_pos()) and player_item and item_type == "pickaxe" and self.parent.active_ui_id is None
        else:
            player_item = 0
            self.touching = False
        ...
        if self.touching and pg.mouse.get_pressed()[0] and not self.just_pressed:
            ...
            if player_item.dont_need_clicking:
                ...
                for i in range(1):
                    ...
                    obj = Particle(self.parent.generate_id(), particle_pos, self.parent, particle_velocity, lifetime,
                                   particle_texture)
                ...
                if not self.just_pressed:
                    ...
                    if self.hp <= 0:
                        item = ORE_TYPE_TO_ITEM[self.ore_type]
                        xvel = choice((TILE_SIZE.x / 16, -(TILE_SIZE.x / 16))) + randint(round(-(TILE_SIZE.x / 32)), int(TILE_SIZE.x / 32))
                        self.parent.drop_items(self.pos, item, 1, Vector(xvel, - TILE_SIZE.y / 8), randint(25, 35))
                        self.hp = ORE_TYPE_TO_HP[self.ore_type]
                        SOUND2.play()
                        return
    ...
    def draw(self):
        ...
        # pg.draw.rect(self.parent.screen, "#ff00ff", self.rect, 3)
        # pg.draw.circle(screen, "#ffffff", self.screen_pos.as_tuple(), 5)
        # pg.draw.circle(screen, "#ff0000", (self.pos+self.parent.offset).as_tuple(), 10)
    ...
    def load(self, data: dict):
        ...
        self.hp = data["hp"]
        self.texture = ORE_TEXTURES.get(self.ore_type, pg.transform.scale(TEXTUTURE_NULL, DEPOSIT_SIZE.as_tuple()))["deposit"]
