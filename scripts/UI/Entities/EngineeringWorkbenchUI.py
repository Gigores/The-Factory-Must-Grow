...
class EngineeringWorkbenchUI(BaseUI, metaclass=UIRegistry):

    def __init__(self, parent):

        from scripts.Managers.GameAssets import engineering_workbench_recipes

        self.workbench_ui_texture = pg.transform.scale_by(pg.image.load("assets/ingame_UI/engineering_workbench_ui.png"), 7)
        self.workbench_pos = (
            RESOLUTION.x / 2 - self.workbench_ui_texture.get_width() / 2,
            RESOLUTION.y / 2 - self.workbench_ui_texture.get_width() / 2
        )
        self.pixel_size = Vector(self.workbench_ui_texture.get_width() // 90, self.workbench_ui_texture.get_height() // 68)
        self.small_frame_size = self.pixel_size * Vector(10, 10)
        recipe_book_btn_pos = (self.workbench_pos[0] - self.RECIPE_BOOK_SIZE.x * 1.2, self.workbench_pos[1] + BLANK2.get_height() / 2 - self.RECIPE_BOOK_SIZE.y / 2)
        ...
        self.recipies_texture = pg.transform.scale(BLANK2, self.recipies_size.as_tuple())
        self.recipies_pos = (
        ...
        self.recipes = {submenu: [] for submenu in engineering_workbench_recipes.keys()}

        self.current_category = list(self.recipes.keys())[0]
        self.current_book_recipie = None

        for menu in self.recipes.keys():
            for submenu in engineering_workbench_recipes[menu]:
                for recipe in engineering_workbench_recipes[menu][submenu]:
                    self.recipes[menu].append(recipe)

        for category_name, category in engineering_workbench_recipes.items():
        ...
        self.inventory_pos = (
            ...
            self.recipies_pos[1] + self.workbench_ui_texture.get_height()
        )
        ...
        self.recipe_window_size = Vector(FRAME_SIZE[0] * 5 + border * 2 + border * 0.5, FRAME_SIZE[1] * 2 + border * 3)
        self.recipe_window_texture = pg.transform.scale(BLANK2, self.recipe_window_size.as_tuple())

        self.exmpl_ingredients_pos = self.recipe_window_pos + Vector(border, border)

        self.exmpl_ingredients_buttons = []
        for n in range(6):
            self.exmpl_ingredients_buttons.append(Button(TEXTUTURE_NULL, TEXTUTURE_NULL, "NULL", FRAME_SIZE, self._get_ingredient_recipe_pos(n).as_tuple()))

        self.exmpl_result_pos = self.recipe_window_pos + Vector(border, border) + Vector(FRAME_SIZE[0] * 4, FRAME_SIZE[1] * 0.5) + Vector(border * 0.5, 0)
        self.exmpl_result_button = Button(TEXTUTURE_NULL, TEXTUTURE_NULL, "NULL", FRAME_SIZE, self.exmpl_result_pos.as_tuple())

        self.arrow_center_pos = self.recipe_window_pos + Vector(border * 2, border) + Vector(FRAME_SIZE[0] * 3.5, FRAME_SIZE[1] * 1)
        ...
        self.result_pos = from_iterable(self.workbench_pos) + self.pixel_size * Vector(63, 24)
        self.result_button = Button(TEXTUTURE_NULL, TEXTUTURE_NULL, "NULL", self.small_frame_size.as_tuple(), self.result_pos.as_tuple())
        self.input_items_buttons = []
        for n in range(6):
            self.input_items_buttons.append(Button(TEXTUTURE_NULL, TEXTUTURE_NULL, "NULL", self.small_frame_size.as_tuple(), self._get_input_items_pos(n).as_tuple()))

    def _get_ingredient_recipe_pos(self, n: int):

        return self.exmpl_ingredients_pos + Vector(FRAME_SIZE[0] * (n % 3), FRAME_SIZE[1] * (n // 3))

    def _get_input_items_pos(self, n: int):

        return from_iterable(self.workbench_pos) + self.pixel_size * Vector(25, 21) + Vector(self.small_frame_size.x * (n % 3), self.small_frame_size.y * (n // 3))
    ...
    def draw(self, display):
        ...
        if self.recipies_opened:
            ...
            if self.current_book_recipie:
                ...
                for n, item in enumerate(self.current_book_recipie.ingredients + [[None, 0] for _ in range(6 - len(self.current_book_recipie.ingredients))]):
                    draw_item(screen, item[0], item[1], self._get_ingredient_recipe_pos(n).as_tuple(), FRAME_SIZE)
            ...
            if self.obj:
                ...
                for i, (n, a) in enumerate(zip(self.obj.input_items.n, self.obj.input_items.a)):
                    draw_item(screen, n, a, self._get_input_items_pos(i).as_tuple(), self.small_frame_size.as_tuple(), draw_bg=False)
                """
                ...
                item_amount = self.obj.output_item.item_amount if self.obj.output_item.item_name else 1
                draw_item(screen, self.obj.output_item.item_name, item_amount,
                          self.result_pos.as_tuple(), FRAME_SIZE, SMALL_FONT, draw_bg=False,
    ...
    def inventory_shift(self, inventory, button_id):
        ...
        if self.obj.input_items.can_fit(inventory.n[button_id], inventory.a[button_id]):
            self.obj.input_items.append(inventory.n[button_id], inventory.a[button_id])
            inventory.pop_from_slot(button_id, inventory.a[button_id])

    def input_shift(self, inventory, button_id):
    ...
    def update(self, obj):
        ...
        if self.recipies_opened:
            ...
            for n, button in enumerate(self.exmpl_ingredients_buttons):
                button.update()
                if button.touching and n < len(self.current_book_recipie.ingredients):
                    self._draw_item_information(self.current_book_recipie.ingredients[n][0])
            ...
            self._handle_inventory(self.obj.input_items, self.input_items_buttons, self.input_shift)
            """
            ...
            """
            self._handle_slot(obj.output_item, self.result_button, self.slot_shift, output_only=True)
            self.current_recipie = engineering_workbench_recipes.find(self.obj.input_items.n, self.obj.input_items.a)
    ...
    def craft(self):
        ...
        ingredients = self.current_recipie.ingredients
        result = self.current_recipie.result

        has_engrs = self.obj.input_items.has_m(ingredients)
        enough_space = self.obj.output_item.can_fit(result[0], result[1])

        if has_engrs and enough_space:
            self.obj.input_items.pop_m(ingredients)
            self.obj.output_item.append(result[0], result[1])
            return
